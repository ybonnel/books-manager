...
describe('BookItem', () => {
  ...
  describe('Instantiation:', () => {
    ...
    it('should initialize #props.book with a Book instance', () => {
      expect(taskItem.props.book instanceof Task).toBe(true);
    });
  });
  ...
  describe('Component methods:', () => {
    describe('#delete', () => {
      it('should call #bookActions.deleteBook', () => {
        taskItem.delete();
        expect(taskItem.props.deleteBook.callCount).toEqual(1);
        expect(taskItem.props.deleteBook.calledWith(taskItem.props.book)).toEqual(true);
      });
    });
    ...
    describe('#saveTitle', () => {
      ...
      it('should call #bookActions.updateBook', () => {
        taskItem.state.editing = true;
        taskItem.saveTitle({
          target: {value: 'foo'}
        });
        expect(taskItem.props.updateBook.callCount).toEqual(1);
        expect(taskItem.props.updateBook.args[0][0]).toEqual(task);
        expect(taskItem.props.updateBook.args[0][1].title).toEqual('foo');
      });
    });
    ...
    describe('#toggleStatus', () => {
      ...
      it('should toggle book.complete', () => {
        taskItem.toggleStatus();
        expect(taskItem.props.updateBook.args[0][1].completed).toEqual(!task.completed);
      });
    });
  ...
  describe('DOM', () => {
    ...
    describe('`blur` event triggered on text field', () => {
      ...
      it('should toggle visibility of text field and book title', () => {
        taskItem.setState({editing: true});
        Simulate.blur(taskItem.titleInput);
